...
(function (global) {
  ...
  PAGES.dailyAuth = function () {
    var A = L.Auth;
    /* 云端模式：先加载服务器配置再渲染 */
    if (L.Cloud && L.Cloud.isCloud() && !A._cloudCfgLoaded()) {
      $('#abody').innerHTML = '<div style="padding:40px;text-align:center;color:var(--ink-400)">加载授权配置中…</div>';
      A.loadCloudCfg().then(function () { PAGES.dailyAuth(); });
      return;
    }
    var ac = A.cfg();
    ...
    $('#grRevokeAll').onclick = function () {
      ui().confirmBox('全部撤销', '将把台账中<b style="color:var(--red)">全部有效授权码</b>置为已撤销。确认？', '全部撤销', true)
        .then(function (v) {
          if (!v) return;
          var doRevoke = (L.Cloud && L.Cloud.isCloud())
            ? L.Cloud.api('/api/auth/revokeAll', { method: 'POST' })
            : A.grants.all().then(function (list) { list.forEach(function (g) { g.status = 'revoked'; }); return A.grants.save(list); });
          doRevoke.then(function () { return L.Auth.state.clear(); })
