...
(function (global) {
  ...
  /* ============================================================
   *  云端模式覆盖（http/https 访问时生效）
   *  将本地存储操作替换为后端 API 调用，实现跨设备数据共享。
   *  离线签名算法（makeCode/verifyCode 等）保持不变，
   *  仅替换"台账读写"与"门禁校验"的数据源。
   * ============================================================ */
  var _cloudCfg = null;
  var _cloudCfgLoaded = false;

  function loadCloudCfg() {
    if (!L.Cloud || !L.Cloud.isCloud()) return Promise.resolve();
    return L.Cloud.api('/api/auth/cfg').then(function (r) {
      if (r.ok && r.cfg) _cloudCfg = r.cfg;
      else _cloudCfg = { enabled: true, days: 30, bind: true, secret: '', seq: 1, autoGrant: false };
      _cloudCfgLoaded = true;
      return _cloudCfg;
    });
  }

  if (L.Cloud && L.Cloud.isCloud()) {

    /* ---- 配置：从服务器读取 / 缓存 ---- */
    cfg = function () {
      return _cloudCfg || { enabled: true, days: 30, bind: true, secret: '', seq: 1, autoGrant: false };
    };
    saveCfg = function (patch) {
      return L.Cloud.api('/api/auth/cfg', { method: 'POST', body: patch }).then(function (r) {
        if (r.ok && r.cfg) _cloudCfg = r.cfg;
        return r;
      });
    };
    /* secret 仍用本地缓存（签名算法前后端一致即可） */
    secret = function () { return (cfg().secret || '').trim() || BUILTIN_SECRET; };

    /* ---- 门禁校验：直接查服务器 ---- */
    check = function () {
      if (L.UI && L.UI.isAdminLoggedIn && L.UI.isAdminLoggedIn())
        return Promise.resolve({ ok: true, reason: 'admin' });
      return L.Cloud.api('/api/auth/status').then(function (r) {
        if (r.ok && r.reason === 'grant')
          return { ok: true, reason: 'grant', expireAt: r.expireAt, code: r.code };
        if (r.reason === 'off')
    ...
    redeem = function (code) {
      return L.Cloud.api('/api/auth/redeem', { method: 'POST', body: { code: code } }).then(function (r) {
        return r;  // { ok, reason, msg, expireAt }
      });
    };
    ...
    apply = function (note) {
      var s = (L.UI && L.UI.session && L.UI.session()) || null;
      return L.Cloud.api('/api/auth/apply', { method: 'POST', body: {
        note: note || '',
        name: (s && s.name) || '',
        no: (s && s.no) || '',
        dept: (s && s.dept) || ''
      }}).then(function (r) {
        if (r.ok) return { rec: r.rec, code: r.reqCode };
        return { rec: null, code: '', err: r.msg || '提交失败' };
      });
    };

    /* ---- 申请台账（管理员） ---- */
    reqs.all = function () {
      return L.Cloud.api('/api/auth/requests').then(function (r) {
        return r.ok ? (r.requests || []) : [];
      });
    };
    reqs.save = function () { return Promise.resolve(); };
    reqs.add = function (item) {
      /* 管理员粘贴申请码 → 重建 reqCode → 交服务器解析入库 */
      var reqCode = buildReqCode(item);
      return L.Cloud.api('/api/auth/importReq', { method: 'POST', body: { reqCode: reqCode } }).then(function (r) {
        return r.ok ? r.rec : null;
      });
    };
    reqs.update = function (id, patch) {
      if (patch.status === 'rejected')
        return L.Cloud.api('/api/auth/deny', { method: 'POST', body: { reqId: id } });
      /* 'granted' 由 grant API 在服务端自动处理，此处 no-op */
      return Promise.resolve({});
    };
    reqs.remove = function (id) {
      return L.Cloud.api('/api/auth/req/' + encodeURIComponent(id), { method: 'DELETE' });
    };
    reqs.clearDone = function () {
      return L.Cloud.api('/api/auth/clearReqs', { method: 'POST' });
    };

    /* ---- 授权台账（管理员） ---- */
    grants.all = function () {
      return L.Cloud.api('/api/auth/grants').then(function (r) {
        return r.ok ? (r.grants || []) : [];
      });
    };
    grants.save = function () { return Promise.resolve(); };
    grants.issue = function (o) {
      if (o.reqId) {
        return L.Cloud.api('/api/auth/grant', { method: 'POST', body: {
          reqId: o.reqId, days: o.days, bindUser: o.bindUser
        }}).then(function (r) { return r.ok ? r.grant : null; });
      }
      return L.Cloud.api('/api/auth/grant/manual', { method: 'POST', body: {
        user: o.bindUser || '', name: o.name || '', no: o.no || '', dept: o.dept || '',
        days: o.days, note: o.note || '手动生成'
      }}).then(function (r) { return r.ok ? r.grant : null; });
    };
    grants.update = function (code, patch) {
      if (patch.status === 'revoked')
        return L.Cloud.api('/api/auth/revoke', { method: 'POST', body: { code: code } });
      if (patch.status === 'active')
        return L.Cloud.api('/api/auth/restore', { method: 'POST', body: { code: code } });
      return Promise.resolve({});
    };
    grants.extend = function (code, days) {
      return L.Cloud.api('/api/auth/extend', { method: 'POST', body: { code: code, days: days } }).then(function (r) {
        return r.ok ? r.grant : null;
      });
    };
    grants.remove = function (code) {
      return L.Cloud.api('/api/auth/grant/' + encodeURIComponent(code), { method: 'DELETE' });
    };
    /* revokeAll：覆盖 save 的特殊场景由 admin.js 直接调 API */
  }
  ...
  L.Auth = {
    ...
    loadCloudCfg: loadCloudCfg, _cloudCfgLoaded: function () { return _cloudCfgLoaded; }
  };
