...
(function (global) {
  ...
  function saveRecord(rec) {
    ...
    if (L.Cloud && L.Cloud.isCloud()) {
      var payload = {
        ...
        combo: rec.category || '',
        title: rec.title || '',
        passScore: rec.passScore || 70,
        duration: rec.duration || Math.round(((rec.used || 0)) / 1000),
        detail: rec.detail || [],
        result: { score: rec.score, total: rec.total, right: rec.right, wrong: rec.wrong, blank: rec.blank }
      };
      var cloud = L.Cloud.api('/api/exam/submit', { method: 'POST', body: payload });
  ...
  function initSession() {
    if (L.Cloud && L.Cloud.isCloud()) {
      var info = L.Cloud.getUserInfo();
      if (!info) { _session = null; updateUserChip(); return Promise.resolve(null); }
      return L.Cloud.api('/api/me').then(function (r) {
        if (r.ok && r.user) {
          _session = { user: r.user.user, name: r.user.name, no: r.user.no || '', dept: r.user.dept || '' };
        } else {
          L.Cloud.clearToken();
          _session = null;
        }
  ...
  function doLogout() {
    _session = null;
    if (L.Cloud && L.Cloud.isCloud()) L.Cloud.logout();
    L.Store.del('session');
    updateUserChip();
    go('login');
  }
  ...
  function initAdminSession() {
    if (L.Cloud && L.Cloud.isCloud()) {
      var info = L.Cloud.getAdminInfo();
      _adminSession = info || null;
      return Promise.resolve(_adminSession);
    }
  ...
  function adminLogout() {
    _adminSession = null;
    if (L.Cloud && L.Cloud.isCloud()) L.Cloud.adminLogout();
  ...
  Views.adminLogin = function () {
    ...
    function tryLogin() {
      ...
      setErr('');
      if (L.Cloud && L.Cloud.isCloud()) {
        L.Cloud.adminLogin(user, pw).then(function (r) {
          if (!r.ok) { setErr(r.msg || '登录失败'); return; }
          adminLogin({ user: r.admin.user, name: r.admin.name }).then(function () { L.Admin.enter(); });
        });
      } else {
  ...
  Views.login = function () {
    ...
    function tryLogin() {
      ...
      setErr('loginErr', '');
      if (L.Cloud && L.Cloud.isCloud()) {
        L.Cloud.login(user, pw).then(function (r) {
          if (!r.ok) { setErr('loginErr', r.msg || '登录失败'); return; }
          doLogin({ user: r.user.user, name: r.user.name, no: r.user.no || '', dept: r.user.dept || '', guest: false });
        });
      } else {
    ...
    function tryReg() {
      ...
      if (L.Cloud && L.Cloud.isCloud()) {
        ...
        L.Cloud.register(user, pw, name, ($('#rNo').value || '').trim(), ($('#rDept').value || '').trim()).then(function (r) {
          if (!r.ok) { setErr('regErr', r.msg || '注册失败'); return; }
          doLogin({ user: r.user.user, name: r.user.name, no: r.user.no || '', dept: r.user.dept || '', guest: false });
        });
      } else {
        if (!ans) return setErr('regErr', '请填写密保答案');
  ...
  Views.kpExamSetup = function () {
    ...
    var state = { type: 'first', main: 'top', vice: '', post: 'top', major: Object.keys(majors)[0] || '', cat: '' };
    if (state.major && majors[state.major]) state.cat = majors[state.major][0].id;
    // 直达链接预填（?post=tech&exam=1|2&cat=<二级类别id>）
    if (_deep && _deep.post && L.Bank.getPosition(_deep.post)) {
      state.main = _deep.post;
    }
    if (_deep && _deep.exam) state.type = (_deep.exam === '2' ? 'extend' : 'first');
    if (_deep && _deep.cat) {
    ...
    function render() {
      ...
      setHTML(
        ...
        '<div class="nm">首次考试</div><div class="ds">按报考岗位大纲配比组卷（含科目 A/B/C' + (needD ? ' 与科目D' : '') + '）；单选 1 分、多选 2 分、判断 2 分，合计 100 分</div>' +
        '<div class="qs">时长 ' + cfg.firstMin + ' 分钟　合格线 ' + cfg.passScore + ' 分</div></div>' +
        '<div class="pick' + (state.type === 'extend' ? ' on' : '') + '" data-type="extend">' +
        '<div class="nm">扩领域考试</div><div class="ds">仅考科目D 一个专业大类：单选 20×2 分、多选 20×2 分、判断 10×2 分</div>' +
      ...
      $$('[data-type]').forEach(function (el) { el.onclick = function () { state.type = el.getAttribute('data-type'); if (state.type === 'extend') state.vice = ''; render(); }; });
      var sv = $('#selVice');
      ...
      $('#paperInfo').innerHTML = '本场报考：<b>' + esc(pos.name) + '</b>　·　' + (isExt2 ? '扩领域考试（仅科目D）' : '首次考试') +
        '　·　组卷：' + esc(L.Engine.planSummary(infoPlan)) +
        '　·　时长 ' + (isExt2 ? cfg.extendMin : cfg.firstMin) + ' 分钟　·　合格线 ' + cfg.passScore + ' 分';
